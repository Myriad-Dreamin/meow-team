...
function composeResolvePrompt(plan, options) {
  return [
    ...
    "4. Update `pnpm-lock.yaml` before pushing when package manifests or workspace dependency versions changed. Run `pnpm install --lockfile-only` or the appropriate pnpm install command, then include the lockfile in the merge commit. A stale lockfile makes CI fail immediately and costs an extra fix round.",
    "5. Run `npm run format`, `npm run lint`, and `npm run typecheck` after resolving conflicts. Run focused tests for files you materially change.",
    `6. Commit the result if needed. Preferred commit title: ${plan.title}`,
    `7. Push the branch with: git push -u ${options.originRemote} ${plan.branchName}`,
    "",
    "Useful checks:",
