...
### Requirement: `meow-flow plan` loads a team config module

The `meow-flow plan` command SHALL load team configuration from an explicit `--config` path when provided, or else discover `team.config.ts` by searching from the current working directory toward the filesystem root.
...
- **WHEN** a user runs `meow-flow plan` inside a repository tree that contains `team.config.ts` in the current directory or an ancestor directory
...
- **WHEN** a user runs `meow-flow plan` without `--config` and no `team.config.ts` can be discovered
...
`meow-flow plan` SHALL derive the repository candidate set from the normalized team config instead of scanning arbitrary directories.
...
- **THEN** `meow-flow plan` excludes them from the candidate set
...
`meow-flow plan` SHALL emit deterministic worktree allocation data for each selected repository so future execution commands can reuse the same planning result.
...
- **WHEN** a user runs `meow-flow plan --json`
