...
- [x] 1.1 Review the approved OpenSpec artifacts for "Replace the team request
      textarea with a CodeMirror request editor" and confirm the canonical
      request/PR title is `feat: replace team request editor`
- [x] 1.2 Confirm conventional-title metadata `feat` stays separate from
...
- [x] 2.1 Refactor `lib/team/execution-mode.ts` so parser matching,
...
- [x] 2.2 Update `lib/team/execution-mode.test.ts` and any related coverage so
...
- [x] 3.1 Extract or introduce a reusable CodeMirror wrapper from
...
- [x] 3.2 Replace the `Request` textarea in `components/team-console.tsx` with
