...
- Update config resolution so `--config <path>` still wins, and otherwise only the shared installed config is loaded; commands fail clearly when no shared config has been installed.
...
- `packages/meow-flow` tests and README for install behavior, source-type support, shared-config precedence, and missing-install diagnostics
