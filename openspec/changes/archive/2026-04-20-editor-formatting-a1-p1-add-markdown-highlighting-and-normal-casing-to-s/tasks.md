...
- [x] 1.1 Update `components/codemirror-text-editor.tsx` to load the required
  CodeMirror markdown mode without disturbing the current addon import order.
- [x] 1.2 Confirm the shared editor configuration still preserves request
  prefix and slash-command autocomplete behavior after markdown mode is wired.

## 2. Editor presentation

- [x] 2.1 Add shared CSS rules in `app/globals.css` for
...
- [x] 2.2 Verify `components/team-request-editor.tsx` and
...
- [x] 3.1 Extend focused tests around the shared editor dependency/runtime so
  markdown mode packaging regressions are caught.
- [x] 3.2 Update adjacent editor tests such as
