...
- Add `meow-flow run <thread>` as the first thread allocation command.
...
- `thread-workspace-occupancy`: Allocate fixed Paseo worktree slots to named Meow Flow threads and persist those occupations locally.
...
- `packages/meow-flow` command tree, thread listing/allocation logic, SQLite storage module, README, and CLI tests.
- `packages/meow-flow/package.json`, root `package.json` pnpm build-dependency allowlist, and `pnpm-lock.yaml` for `better-sqlite3`.
- No Paseo daemon, WebSocket, mobile-app, or message-schema changes.
