...
- [ ] 2.7 Implement transactional delete helpers that validate all requested thread ids before removing any occupations.
...
- [ ] 3.6 Add `meow-flow delete <id1> <id2> ...` with repository-independent SQLite lookup, all-or-nothing deletion, and released-workspace output.
- [ ] 3.7 Update `packages/meow-flow/README.md` and CLI help expectations for `run`, `delete`, `thread ls`, and the `ls` alias.
...
- [ ] 4.3 Add targeted CLI tests for `meow-flow delete` single-id release, multi-id release, missing-id all-or-nothing failure, and post-delete idle listing.
- [ ] 4.4 Add targeted CLI tests for `meow-flow ls` alias behavior and stale occupation handling.
- [ ] 4.5 Run the changed `meow-flow` test file(s) only with `npx vitest run <file> --bail=1`.
- [ ] 4.6 Run `npm run format` and `npm run typecheck` after implementation.
