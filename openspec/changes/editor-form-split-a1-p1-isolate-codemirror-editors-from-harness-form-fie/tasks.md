...
- [ ] 1.1 Refactor `app/globals.css` so `.harness-form-field` stays layout-only
      and caption/native `input`/`select`/`textarea` styling moves to explicit
      harness native-field hooks.
- [ ] 1.2 Update `components/team-console.tsx`,
...
- [ ] 2.1 Verify `components/thread-command-composer.tsx`,
...
- [ ] 2.2 Confirm the planner request editor and thread command editor keep
      their current placeholder, disabled-state, focus, and helper-copy behavior
      after the form primitive split.

## 3. Regression coverage

- [ ] 3.1 Extend focused tests around the harness form CSS contract and the
      adjacent request or command components so reintroducing broad descendant
      selectors fails quickly.
- [ ] 3.2 Update targeted editor or form tests to prove native controls retain
