...
- [ ] 1.1 Update `lib/git/ops.ts` so branch-head lookup prefers explicit local
...
- [ ] 1.2 Extend `lib/git/ops.test.ts` with repo-backed regression coverage for
  nested request branch namespaces and compatibility fallback inputs.

## 2. Approval Pipeline Adoption

- [ ] 2.1 Reuse the hardened shared branch-head lookup anywhere approval-time
  branch refresh depends on it, especially proposal approval and final
  archive or PR-refresh paths under `lib/team/coding/*`.
- [ ] 2.2 Add approval-flow regression coverage in `lib/team/coding/index.test.ts`
  for the reported
...
- [ ] 3.1 Run the targeted git-helper and approval-flow tests that cover the
  new slash-delimited branch cases.
- [ ] 3.2 Run `pnpm fmt`, `pnpm lint`, and `pnpm build` before handoff when the
