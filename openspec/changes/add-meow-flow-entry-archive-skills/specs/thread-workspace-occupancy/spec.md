...
### Requirement: `mfl run` allocates a thread id to an idle workspace
...
MeowFlow workspace for the canonical repository root, store the request body,
...
- **WHEN** a user runs `mfl run --id fix-test-ci "echo \"hello world\""`
  inside a git repository with three configured linked workspaces
- **AND** the first workspace already has a running occupation
- **AND** the second workspace is registered as a Git worktree and has no
  running occupation
- **THEN** the CLI persists a running occupation for thread id `fix-test-ci`
  and the second workspace
...
- **AND** a registered linked workspace has no running occupation
- **THEN** the CLI generates a random UUID thread id
- **AND** the CLI persists the generated thread id for that workspace
...
- **WHEN** a user runs `mfl run --id fix-test-ci "echo \"hello world\""`
- **AND** the first linked workspace already has a running occupation
- **AND** the second linked workspace is not registered as a Git worktree
- **AND** the third linked workspace is registered as a Git worktree and has no
  running occupation
- **THEN** the CLI allocates the third linked workspace
- **AND** the CLI does not create the second linked workspace

#### Scenario: No idle registered workspace is available

- **WHEN** a user runs `mfl run --id fix-test-ci "echo \"hello world\""`
- **AND** every configured registered linked workspace is occupied or
  not-created
...
the thread and stage. The invocation uses the target workspace as cwd, includes
...
- **WHEN** a user runs `mfl run --id fix-test-ci "echo \"hello world\""`
- **AND** the second linked workspace is selected as the idle registered
  workspace
- **THEN** the CLI invokes `paseo run` with cwd set to the absolute workspace
  path
...
The CLI SHALL fail rather than launch a new thread in a workspace that already
has a running MeowFlow occupation. The CLI SHALL allow `mfl run --stage
<stage>` from the occupied workspace only when the requested stage belongs to
...
- **WHEN** `mfl run --id add-feature "echo \"hello world\""` selects a linked
  workspace as the candidate workspace
- **AND** another command records a running occupation for that workspace before
...
#### Scenario: Only registered workspace is already running a different thread

- **WHEN** a user runs `mfl run --id add-feature "echo \"hello world\""`
- **AND** the only configured registered linked workspace already has a running
...
#### Scenario: Same-thread stage launch is allowed in an occupied workspace
...
- **THEN** the command launches a `meow-code` agent in the same workspace
- **AND** the command does not allocate another workspace
