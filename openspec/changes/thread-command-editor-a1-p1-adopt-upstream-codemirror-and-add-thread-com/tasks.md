...
- [x] 1.1 Review the approved OpenSpec artifacts for "Adopt upstream CodeMirror
...
- [x] 2.1 Replace the workspace-linked `codemirror` dependency and remove the
      local shim path from the editor integration, lockfile contract, and
      review guards
- [x] 2.2 Move supported thread-command metadata into
...
- [x] 3.1 Rework `components/thread-command-editor.tsx` around the upstream
...
- [x] 3.2 Add assignment-aware autocomplete for `/approve`, `/ready`,
...
- [x] 4.1 Update `app/globals.css` and focused tests for the upstream editor
...
- [x] 4.2 Run `pnpm fmt`, `pnpm lint`, targeted `pnpm test`, and `pnpm build`
