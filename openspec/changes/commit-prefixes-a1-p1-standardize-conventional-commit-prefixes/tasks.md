...
- [ ] 1.1 Add a shared harness commit-message formatter or classifier that
      emits lowercase `docs:`, `dev:`, `fix:`, and `test:` subjects and defaults
      ambiguous internal work to `dev:`
- [ ] 1.2 Add focused unit coverage for deterministic mapping, including
...
- [ ] 2.1 Update `lib/team/openspec.ts` so planner proposal materialization
      commits use the shared formatter and emit `docs:` instead of `planner:`
- [ ] 2.2 Update `lib/team/coding/reviewing.ts` so implementation, proposal or
...
- [ ] 3.1 Update coder lane prompt guidance so any direct `git commit` commands
      use the same lowercase conventional prefix rules as harness-managed commits
- [ ] 3.2 Update reviewer and archive-oriented prompt guidance so direct commit
...
- [ ] 4.1 Extend `lib/git/ops-materialization.test.ts`,
...
- [ ] 4.2 Run `pnpm fmt`, `pnpm lint`, and the relevant automated tests for the
