...
This change captures proposal "Split `network.ts` into stage-oriented `lib/team/coding` modules" as OpenSpec change `coding-stages-a1-p1-split-network-ts-into-stage-oriented-lib-team-coding`.
...
- Keep the canonical request/PR title as `refactor(team/coding): Split `network.ts` into stage-oriented`.
