# static-role-prompts-a2-p1-statically-inline-role-system-prompts Specification

## Purpose
Statically inline the harness role system prompts through `prompts/roles/*.prompt.md`
modules and a typed registry so runtime filesystem reads are removed while role
prompt authoring stays in `prompts/roles`.
## Requirements
