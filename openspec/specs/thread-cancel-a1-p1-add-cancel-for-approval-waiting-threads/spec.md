# thread-cancel-a1-p1-add-cancel-for-approval-waiting-threads Specification

## Purpose

Define the thread-scoped `/cancel` command that lets the owner stop the latest
idle approval-waiting assignment, persists a terminal `cancelled` lifecycle
state for that request group, and renders `Cancelled` consistently across
history, archive gating, docs, and UI.
