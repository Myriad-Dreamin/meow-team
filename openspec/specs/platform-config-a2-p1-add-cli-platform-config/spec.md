# platform-config-a2-p1-add-cli-platform-config Specification

## Purpose
Define the repository-local platform configuration capability for the
`meow-team` CLI, including Clipanion command bootstrap, `git config --local`
persistence under `meow-team.platform`, and repository-aware platform
resolution that defaults to GitHub while rejecting unsupported configured
providers.
## Requirements
