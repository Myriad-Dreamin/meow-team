# platform-config-a1-p1-add-cli-platform-config Specification

## Purpose
Define repository-local git platform selection for the standalone
`meow-team` CLI so each repository can persist its adapter choice, default to
GitHub when unset, and fail explicitly for configured but unsupported
platforms such as `ugit`.
