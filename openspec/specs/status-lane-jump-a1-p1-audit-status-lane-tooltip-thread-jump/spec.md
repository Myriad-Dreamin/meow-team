# status-lane-jump-a1-p1-audit-status-lane-tooltip-thread-jump Specification

## Purpose

Define the regression-follow-up status-lane thread-jump capability for the
workspace status bar so non-zero lane pills keep revealing matching living
threads, preserve same-thread multiplicity context, and reuse the existing
thread-tab selection flow when a popover row is activated.
