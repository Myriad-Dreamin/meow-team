# ugit-pr-links-a1-p1-fix-ugit-browser-pr-links Specification

## Purpose

Ensure ugit browser links resolve from repository-local browser-server
configuration and stable repository metadata so pull requests open at
`http://localhost:17121/owner/repo/pull-requests/<id>` by default instead of
reusing local-path or ssh transport URLs, while keeping GitHub link behavior
unchanged.
## Requirements
