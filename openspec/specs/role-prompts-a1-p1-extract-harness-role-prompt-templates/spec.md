# role-prompts-a1-p1-extract-harness-role-prompt-templates Specification

## Purpose

Define the harness role prompt-template capability so `lib/team/roles` can use
colocated typed `meow-prompt` markdown templates while preserving the separate
role-definition prompts in `prompts/roles`.
