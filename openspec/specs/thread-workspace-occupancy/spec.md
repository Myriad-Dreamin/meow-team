...
The CLI SHALL provide `mfl run [--id <id>] "request body"` for use from inside a git-managed folder and SHALL allocate the resolved thread id to an existing registered `.paseo-worktrees/paseo-N` workspace for the canonical repository root before launching a Paseo agent for that workspace.

#### Scenario: Explicit id allocates the first idle registered slot

- **WHEN** a user runs `mfl run --id fix-test-ci "echo \"hello world\""` inside a git repository whose config sets `dispatch.maxConcurrentWorkers` to `3`
...
After allocating a workspace, `mfl run [--id <id>] "request body"` SHALL invoke `paseo run` with the allocated workspace as cwd, a label `x-meow-flow-id=<resolved-id>`, and the request body passed through unchanged.
...
- **WHEN** `mfl run --id fix-test-ci "echo \"hello world\""` reserves `.paseo-worktrees/paseo-2`
...
- **WHEN** `mfl run --id add-feature "echo \"hello world\""` selects `.paseo-worktrees/paseo-2` as the candidate workspace
...
- **WHEN** `mfl run --id fix-test-ci "echo \"hello world\""` has allocated `.paseo-worktrees/paseo-2`
