# thread-command-a1-p1-add-thread-slash-command-composer-and-executor Specification

## Purpose

Define the thread-scoped slash-command surface that lets the owner submit
`/approve`, `/ready`, `/replan`, and `/replan-all` against the latest idle
assignment while reusing the existing approval and replanning flows.
