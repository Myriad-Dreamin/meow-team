...
// Workspace scripts only add the local node_modules/.bin to PATH; hoisted
...
function workspaceRunCommand(workspace: string, script: string): string {
  return `pnpm --dir ${JSON.stringify(repoRoot)} --filter ${workspace} run ${script}`;
}
...
async function writeJsonSummary({
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        suite: "cli-local",
        command: "pnpm --filter @getpaseo/cli run test:local",
        counts: {
          passed,
          failed,
          skipped: 0,
  ...
  try {
    return await new Promise<TestOutcome>((resolve) => {
      const proc = spawn("npx", ["tsx", testPath], {
        env: {
          ...process.env,
          PATH: [cliBinDir, rootNodeModulesBin, process.env.PATH].filter(Boolean).join(delimiter),
