...
export function loadPersistedConfig(paseoHome: string, logger?: LoggerLike): PersistedConfig {
  ...
  if (!existsSync(configPath)) {
    try {
      ...
      throw Object.assign(new Error(`[Config] Failed to initialize ${configPath}: ${message}`), {
        cause: err,
      });
    }
  }
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    throw Object.assign(new Error(`[Config] Failed to read ${configPath}: ${message}`), {
      cause: err,
    });
  }
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    throw Object.assign(new Error(`[Config] Invalid JSON in ${configPath}: ${message}`), {
      cause: err,
    });
  }
...
export function savePersistedConfig(
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    throw Object.assign(new Error(`[Config] Failed to write ${configPath}: ${message}`), {
      cause: err,
    });
  }
}
