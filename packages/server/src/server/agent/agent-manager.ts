...
const BUSY_STATUSES = new Set<AgentLifecycleStatus>(["initializing", "running"]);
    ...
    if (normalized.cwd) {
      ...
      try {
        ...
        if (
          ...
          throw Object.assign(new Error(`Working directory does not exist: ${normalized.cwd}`), {
            cause: error,
          });
        }
        ...
        throw Object.assign(new Error(`Failed to access working directory: ${normalized.cwd}`), {
          cause: error,
        });
      }
    }
