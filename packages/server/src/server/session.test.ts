...
describe("session checkout commit handling", () => {
  ...
  test("adds configured title guidance when generating commit messages", async () => {
    ...
    checkoutGitMocks.commitChanges.mockResolvedValue(undefined);
    const session = createSessionForTest({ workspaceGitService, messages });
    const checkoutSession = session as unknown as {
      handleCheckoutCommitRequest(input: {
        type: "checkout_commit_request";
        cwd: string;
        message: string;
        addAll: boolean;
    ...
    await checkoutSession.handleCheckoutCommitRequest({
      type: "checkout_commit_request",
      cwd: "/tmp/request-worktree",
      message: "",
      addAll: true,
...
describe("session checkout pull request creation", () => {
  ...
  test("adds recent commit title examples when generating PR text", async () => {
    ...
    const session = createSessionForTest({ workspaceGitService, messages });
    const checkoutSession = session as unknown as {
      handleCheckoutPrCreateRequest(input: {
        type: "checkout_pr_create_request";
        cwd: string;
        baseRef: string;
        title: string;
    ...
    await checkoutSession.handleCheckoutPrCreateRequest({
      type: "checkout_pr_create_request",
      cwd: "/tmp/request-worktree",
      baseRef: "main",
      title: "",
