...
async function resolveDictationConfig(): Promise<DictationConfig> {
  const enabled = resolvePlaywrightSpeechEnabled(process.env);
  if (!enabled) {
    console.log(
      "[e2e] Speech features disabled for Playwright run. Set E2E_ENABLE_SPEECH=1 to enable dictation and voice mode.",
    );
    return { enabled, openAiUsable: false, localModelsDir: null };
  }
  ...
  if (!openAiUsable && !hasDefaultLocalModelsDir) {
    ...
    return { enabled: false, openAiUsable: false, localModelsDir: null };
  }
  ...
  return { enabled, openAiUsable, localModelsDir };
}
