...
function ensureRelayBuildArtifact(repoRoot: string): void {
  ...
  execSync("pnpm --filter @getpaseo/relay run build", {
    cwd: repoRoot,
    stdio: "inherit",
  });
}
...
async function resolveDictationConfig(): Promise<DictationConfig> {
  ...
  if (!speechEnabled) {
    ...
    return { speechEnabled, openAiUsable: false, localModelsDir: null };
  }
  ...
  return { speechEnabled, openAiUsable, localModelsDir };
}
...
function startDaemon(args: DaemonSpawnArgs): ChildProcess {
  ...
  const { speechEnabled, openAiUsable, localModelsDir } = args.dictation;
